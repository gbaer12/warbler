...
class UserModelTestCase(TestCase):
    ...
    def test_valid_signup(self):
        u_test = User.signup('test3', 'test3#test.coom', 'password', None)
        uid = 99999
        u_test.id = uid
        db.session.commit()
    ...
    def test_invalid_email_signup(self):
        invalid = User.signup('test5', None, 'password', None)
        uid = 1234888
        invalid.id = uid
        with self.assertRaises(exc.IntegrityError) as context:
            db.session.commit()
    ...
    def test_invalid_password_signup(self):
        with self.assertRaises(ValueError) as context:
            User.signup('test6', 'test6@email.com', '', None)

        with self.assertRaises(ValueError) as context:
            User.signup('test6', 'test6@test.com', None, None)
